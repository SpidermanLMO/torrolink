      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
          <div style="background:#0f6b6b;padding:24px;border-radius:12px 12px 0 0;">
            ...
            <p style="color:rgba(255,255,255,0.8);margin:4px 0 0;">${business || "Your Business"} • Powered by Torrolink</p>
          </div>
          ...
          <div style="padding:24px;background:#fff;border:1px solid #e2e6ea;">
            ...
            <ul>${topDays.map(d => `<li>${d.day}: <strong>${d.count} scans</strong></li>`).join("")}</ul>
            ` : ""}

            <!-- DEVICES -->
            ${topDevices.length ? `
            <h3 style="color:#0f6b6b;">Devices</h3>
            <ul>${topDevices.map(d => `<li>${d.device}: <strong>${d.percent}%</strong></li>`).join("")}</ul>
            ` : ""}

            <!-- LOCATIONS -->
            ${topLocations.length ? `
            <h3 style="color:#0f6b6b;">Top Locations</h3>
            <ul>${topLocations.map(l => `<li>${l.city}, ${l.state}: <strong>${l.count} scans</strong></li>`).join("")}</ul>
            ` : ""}
