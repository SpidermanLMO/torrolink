...
exports.handler = async (event) => {
  ...
  const { error: insertErr } = await supabaseAnon
    ...
    .insert({
      ...
      comment:      message ? message.trim() : null,
  ...
  const ownerEmail   = profileRes.data && profileRes.data.customers ? profileRes.data.customers.email : null;
  const businessName = (profileRes.data && profileRes.data.business_name) || "Your Business";
  const weekScans    = (scanRes.data && scanRes.data.length) || 0;
...
function buildLeadEmail({ businessName, name, phone, email, message, interests, weekScans }) {
  ...
  if (Array.isArray(interests) && interests.length) {
    interestItems = interests.map(function(i) {
      return '<span style="display:inline-block;background:#e8f5f5;color:#0f6b6b;font-size:13px;font-weight:600;border-radius:20px;padding:4px 12px;margin:3px 4px 3px 0;">' + escHtml(i) + '</span>';
    }).join("");
  }
  const interestsBlock = interestItems
    ? '<div style="margin-top:10px;"><span style="font-size:12px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:.06em;">Interested in:</span><div style="margin-top:6px;">' + interestItems + '</div></div>'
    : "";

  const msgBlock = message
    ? '<div style="background:#f8f8f8;border-radius:8px;padding:14px 16px;margin-top:12px;font-style:italic;color:#444;font-size:14px;line-height:1.6;">&ldquo;' + escHtml(message) + '&rdquo;</div>'
    : "";

  const convRate = weekScans > 0 ? (1 / weekScans * 100).toFixed(1) + "%" : "—";
  ...
  return "<!DOCTYPE html>" +
    ...
    "<tr><td style=\"background:#0f6b6b;padding:22px 28px;\">" +
    ...
    "<div style=\"font-size:14px;color:#888;margin-bottom:24px;\">Someone scanned your QR code and left their contact info &mdash; follow up while it is fresh.</div>" +
    ...
    "<div style=\"font-size:11px;color:#aaa;margin-top:3px;\">QR scans</div></td>" +
    "<td width=\"8px\"></td>" +
    "<td width=\"33%\" style=\"text-align:center;background:#f8f8f8;border-radius:8px;padding:14px 8px;\">" +
    "<div style=\"font-size:26px;font-weight:700;color:#f4752b;\">+1</div>" +
    "<div style=\"font-size:11px;color:#aaa;margin-top:3px;\">New lead today</div></td>" +
    "<td width=\"8px\"></td>" +
    "<td width=\"33%\" style=\"text-align:center;background:#f8f8f8;border-radius:8px;padding:14px 8px;\">" +
    "<div style=\"font-size:26px;font-weight:700;color:#0f6b6b;\">" + convRate + "</div>" +
    "<div style=\"font-size:11px;color:#aaa;margin-top:3px;\">Conversion (wk)</div></td>" +
    "</tr></table>" +
    "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"margin-bottom:28px;\">" +
    "<tr><td align=\"center\">" +
    "<a href=\"https://torrolink.com/portal\" style=\"display:inline-block;background:#0f6b6b;color:#ffffff;font-size:14px;font-weight:700;padding:14px 36px;border-radius:8px;text-decoration:none;\">View leads &amp; dashboard &rarr;</a>" +
    "</td></tr></table>" +
    "</td></tr>" +
    "<tr><td style=\"border-top:1px solid #f0f0f0;padding:16px 28px;text-align:center;\">" +
